...
def run_corrected_age_page():
    ...
    st.markdown(
        f"**Entered GA:** {gestational_age_weeks}+{gestational_age_days} weeks"
    )
    ...
    if chronological_age_days < 0:
        ...
        chronological_months = chronological_age_days / 30.44

        st.write(
            f"📅 Chronological Age: "
            f"**{chronological_weeks} weeks + {chronological_days} days** "
            f"({chronological_months:.1f} months)"
        )

        # GA total days
        gestational_age_total_days = (
            gestational_age_weeks * 7 + gestational_age_days
        )
        ...
        if corrected_age_days >= 0:
            ...
            corrected_months = corrected_age_days / 30.44

            st.success(
                f"✅ Corrected Age: "
                f"**{corrected_weeks} weeks + {corrected_days} days** "
                f"({corrected_months:.1f} months)\n\n"
                "Corrected age is used for growth and developmental assessment "
                "in preterm infants."
            )
            ...
            pma_months = pma_total_days / 30.44

            st.info(
                f"ℹ️ Post-Menstrual Age (PMA): "
                f"**{pma_weeks} weeks + {pma_days} days** "
                f"({pma_months:.1f} months)\n\n"
